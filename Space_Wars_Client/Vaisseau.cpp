...
Vaisseau::Vaisseau(std::string vs, std::string fs)
{
	...
	for (int i = 0; i < 15; i++)
	{
		m_toVBOPoints.push_back(g_vaisseauVertices[i]);
	}
	vbosize = m_toVBOPoints.size() * sizeof(float);
	for (int i = 0; i < 18; i++)
	{
		m_indicesTriangle.push_back(g_vaisseauIndices[i]);
	}
...
void Vaisseau::draw(Mat4x4 projection, Mat4x4 modelView, Mat4x4 world, Point2 color, Point2 camPos, Point2 camDir)
{
