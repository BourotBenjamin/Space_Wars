...
std::unique_ptr<Army> train(int iterations, int threshold, int populationSize, int armySize, int unitLevel, std::vector<std::unique_ptr<Army> >& champions, MyGLWidget* gl)
{
    ...
    while(iterations--) {
        ...
        for(auto itA = armies.begin(); itA != armies.end(); ++itA) {
            ...
            for(auto itB = itA+1; itB != armies.end(); ++itB){
            	int sA, sB;
            	fight(*(itA->army), *(itB->army), sA, sB, gl);
            	itA->score += sA;
            	itB->score += sB;
            }
            for(auto itC = champions.begin(); itC != champions.end(); ++itC) {
                int sA, sC;
                fight(*(itA->army), **itC, sA, sC, gl);
